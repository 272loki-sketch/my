...
const renderUsage = (record, t) => {
  const maxUses = record.max_uses && record.max_uses > 0 ? record.max_uses : 1;
  const usedCount = record.used_count || 0;
  return (
    <Tag color={usedCount >= maxUses ? 'grey' : 'blue'} shape='circle'>
      {usedCount}/{maxUses}
    </Tag>
  );
};
...
export const getRedemptionsColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      render: (_, record) => renderUsage(record, t),
    },
    {
      title: t('兑换人ID'),
