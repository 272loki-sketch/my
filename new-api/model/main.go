...
func createRootAccountIfNeed() error {
	...
	if err := DB.First(&user).Error; err != nil {
		initUsername := os.Getenv("INITIAL_ROOT_USERNAME")
		initPassword := os.Getenv("INITIAL_ROOT_PASSWORD")
		if initUsername == "" {
			initUsername = "root"
		}
		if initPassword == "" {
		...
		common.SysLog("no user exists, create a root user for you: username is " + initUsername)
		hashedPassword, err := common.Password2Hash(initPassword)
		if err != nil {
			return err
		}
		rootUser := User{
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&SuspiciousUserNote{}, "SuspiciousUserNote"},
	}
