...
func AddRedemption(c *gin.Context) {
	...
	if redemption.MaxUses > 10000 {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "单个兑换码可用次数不能超过 10000"})
		return
	}
...
func UpdateRedemption(c *gin.Context) {
	...
	if statusOnly == "" {
		...
		if redemption.MaxUses < cleanRedemption.UsedCount {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "可用次数不能小于已使用次数"})
			return
		}
		if redemption.MaxUses > 10000 {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "单个兑换码可用次数不能超过 10000"})
